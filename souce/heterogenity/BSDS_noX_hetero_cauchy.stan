...
model {
  ...
  //target+= uniform_lpdf(shape|0, 1000);
  target+= cauchy_lpdf(rate|0, 1);
  
  //target+= gamma_lpdf(sigma_y|shape, rate); 
  target+= cauchy_lpdf(sigma_y|0, rate); 

  //target+= multi_normal_cholesky_lpdf(b| sim_mean[1:N_sp], vcv_BSDE);
  target+= multi_normal_lpdf(b| sim_mean[1:N_sp], vcv_BSDE);
  

  target+= normal_lpdf(y| Z * b, Z*sigma_y);
...
vector[N] log_likelihood;
  ...
  for(n in 1:N){log_likelihood[n] = normal_lpdf(y[n]| Z[n,] * b, Z[n,]*sigma_y);}
} 
